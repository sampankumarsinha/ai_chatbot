...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const ChannelMessageInputComponent = () => {
    ...
    const handleSendMessage = async (messageData: any) => {
      // 1. Send message as user to Stream
      try {
      ...
      // 2. Check backend platform to decide if we need to call n8n proxy manually
      // If backend is in 'gemini' mode, the GeminiAgent in the backend will handle it automatically via event listeners.
      try {
        const healthResponse = await fetch(`${backendUrl}/`);
        const healthData = await healthResponse.json();
        
        if (healthData.platform === "gemini") {
          console.log("Backend is in Gemini mode, triggering agent processing...");
          await fetch(`${backendUrl}/process-message`, {
            method: "POST",
        ...
        // If we reach here, we are in n8n mode or fallback
        console.log("Calling n8n webhook via proxy:", messageData.text);
        setIsN8nGenerating(true);
        
        const response = await fetch(`${backendUrl}/proxy-n8n`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            message: messageData.text,
        ...
        if (!response.ok) {
          throw new Error(data.error || data.reason || data.details || "n8n proxy failed");
        }

        const aiReply = data.reply || "No reply received from AI";

        // Inject message back to Stream via backend
        await fetch(`${backendUrl}/inject-ai-message`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            channel_id: channel?.id,
